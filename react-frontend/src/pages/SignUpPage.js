  ...
  const isInputValid = (name, value) => {
    switch (name) {
      ...
      case "confirmPassword":
        return (
          value === user.password && value.length >= 8 && value.length <= 20
        );
      case "email":
      ...
      <Form onSubmit={handleSubmit}>
        <InputGroup>
          ...
          <ValidationIcon isValid={isInputValid("phoneNum", user.phoneNum)}>
            {user.phoneNum.length > 0 ? (
              isInputValid("phoneNum", user.phoneNum) ? (
                <FaCheckCircle />
              ) : (
                <FaTimesCircle />
              )
        ...
        <InputGroup>
          ...
          <ValidationIcon isValid={isInputValid("full_name", user.full_name)}>
            {user.full_name.length > 0 ? (
              isInputValid("full_name", user.full_name) ? (
                <FaCheckCircle />
              ) : (
                <FaTimesCircle />
              )
        ...
        <InputGroup>
          ...
          <ValidationIcon isValid={isInputValid("password", user.password)}>
            {user.password.length > 0 ? (
              isInputValid("password", user.password) ? (
                <FaCheckCircle />
              ) : (
                <FaTimesCircle />
              )
        ...
        <InputGroup>
          ...
          />
          <ValidationIcon
            isValid={isInputValid("confirmPassword", user.confirmPassword)}
          >
            {user.confirmPassword.length > 0 &&
            user.password === user.confirmPassword ? (
              isInputValid("confirmPassword", user.confirmPassword) ? (
                <FaCheckCircle />
              ) : (
                <FaTimesCircle />
              )
        ...
        <Label>
          <HyperLink>Already have an account? Click Here.</HyperLink>
        </Label>
        {registrationSuccess && (
          <SuccessMessage>User registered successfully!</SuccessMessage>
        )}
        <Button
