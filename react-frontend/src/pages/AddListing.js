...
function AddListing() {
	...
	const removeImage = indexToRemove => {
		...
		setListing(prevListing => ({
			...prevListing,
			images: prevListing.images.filter(
	...
	async function submitForm() {
		...
		try {
			...
			try {
				...
				await axios.post(
					process.env.REACT_APP_BACKEND_LINK + "/listings",
					formData,
					{
						headers: {
				...
				const url =
					process.env.REACT_APP_FRONTEND_LINK + "/marketplace";
			...
			<div className="vertical-center add-listing-layout margin">
				<div className="small-container drop-shadow">
					{loading ? (
						...
						<>
							<h2 className="vertical-center no-margin-bottom">
								Post Listing
							</h2>
							<form>
							...
							<p className="vertical-center margin">
								{" "}
								{listing.images.length}/8 images selected
							</p>
				...
				{listing.images.length > 0 && !loading && (
					<div className="flex-container drop-shadow margin">
