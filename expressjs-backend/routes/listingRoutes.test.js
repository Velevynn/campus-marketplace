...
/* global test, expect, require, beforeEach, jest, __dirname */
// Import dependencies
...
test("Posting a listing", async () => {
	...
	const response = await request(app) // eslint-disable-line no-unused-vars
		.post("/")
...
test("Get all listings with query", async () => {
	...
	const response = await request(app).get("/").query({q, page}).expect(200);
